...
function Gnosis:ClearCBTables()
	self.bScan = false;
	self:cleartable( self.scan );
	self:cleartable( self.cb_fl );
...
function Gnosis:FontString( bar, height )
	...
	fs:SetFont(pixelfont, 10, "THINOUTLINEMONOCHROME")
	return fs;
	-- TUKUI SKIN
end
...
function Gnosis:SetBarParams( name, cfgtab, bartab )
	...
	-- TUKUI SKIN
	bar.bar:SetStatusBarTexture( normTex, "BORDER" );
	bar.bar:SetStatusBarColor(.3, .3, .3, 1);
	-- TUKUI SKIN
	...
	if( bShowIcon ) then
		...
		bar.sicon:SetWidth( 1.85 * (tParams.bIconUnlocked and min(tParams.height,tParams.width) or tParams.height) * fScaleIcon );
		bar.sicon:SetHeight( 3.275 * (tParams.bIconUnlocked and min(tParams.height,tParams.width) or tParams.height) * fScaleIcon );
		bar.bicon:ClearAllPoints();
	...
	-- TUKUI SKIN
	if (bShowIcon) then
		local iconPointSide = (tParams.iconside)
		local reverseside = "RIGHT"
		local iconSpacing = 8
		if (iconPointSide == "RIGHT") then
			reverseside = "LEFT"
		elseif (iconPointSide == "LEFT") then
			iconSpacing = -8;
		end
		...
		bar.icon:Width(tParams.height)
		bar.icon:Height(tParams.height)
		
		if not bar.icon.TukBackground then
			bar.icon.TukBackground = CreateFrame("Frame", nil, bar)
			bar.icon.TukBackground:SetTemplate("Default")
			bar.icon.TukBackground:CreateShadow("Default")
			bar.icon.TukBackground:SetBackdropBorderColor(unpack(C["media"].altbordercolor))
			bar.icon.TukBackground:SetFrameStrata(bar:GetFrameStrata())
			bar.icon.TukBackground:SetFrameLevel(bar:GetFrameLevel() - 1)
		end
		
		bar.icon.TukBackground:Point("TOPLEFT", bar.icon, "TOPLEFT", -2, 2)
		bar.icon.TukBackground:Point("BOTTOMRIGHT", bar.icon, "BOTTOMRIGHT", 2, -2)
		
		bar.icon:Point(reverseside, bar, iconPointSide, iconSpacing, 0)
...
function Gnosis:utf8sub(string, i, dots)
...
function Gnosis:CreateCastnameFromString( name, rank, cb )
	...
	-- TUKUI SKIN
	name = self:utf8sub(name, 18, true)
	-- TUKUI SKIN
...
function Gnosis:SetPowerbarValue( cb, curpower, maxpower, bHideSpark )
	local cfg = cb.conf;

	-- set statusbar value
	local val = curpower / maxpower;
	cb.bar:SetValue( val );
	cb:SetAlpha( cfg.alpha );
	cb:Show();
	...
	-- castbar spark
	if( cfg.bShowCBS and not bHideSpark ) then
...
function Gnosis:SetupPowerbar( cb, tiinfo )
	local cname, curpower, maxpower, tiunit, curtimer, icon =
		tiinfo.castname, tiinfo.endTime, tiinfo.duration,
		tiinfo.tiunit, tiinfo.curtimer, tiinfo.icon;
	local barname, cfg = cb.name, cb.conf;
	...
	-- set current value
	self:SetPowerbarValue( cb, curpower, maxpower, curtimer.nodur or curtimer.bNot );
...
function Gnosis:SetupTimerbar( cb, fCurTime, tiinfo )
	...
	local val = bForceStaticDur and 1 or (rem / cb.duration);
	...
	if( cfg.bShowCBS and not (bForceStaticDur and cb.cbs_check) ) then
		cb.cbs:SetPoint( "CENTER", cb.bar, "LEFT", val * cb.barwidth, 0 );
		cb.cbs:Show();
	else
		cb.cbs_hidden = true;
...
function Gnosis:SetupTimerLagBox( cb, showlag, showcasttime, castname, recast, bRecalcTick )
	...
	if( recast and self.tBaseSpell ) then
		local dur = cb.dur and cb.dur or cb.duration;
		if( bRecalcTick or not cb.ticktime ) then
			local _, _, _, _, _, _, curct = GetSpellInfo( self.tBaseSpell.id );
			if( curct ) then
				local fHaste = max( self.tBaseSpell.ct/curct, 1.0 );
...
function Gnosis:SetupCastbar( cb, bIsChannel, fCurTime )
	...
	if( cs and cs.ben and not(cs.bhidenonplayer and cfg.unit ~= "player") ) then
		...
		local shownticks = cs.bars;

		if( cfg.unit == "player" and cs.baddticks and cs.hastespell and cs.basecasttime ) then
			-- estimate amount of ticks due to haste
			local _, _, _, _, _, _, castTime = GetSpellInfo( cs.hastespell );
			if ( castTime ) then
				local haste = cs.basecasttime / castTime;
				noninitticks = floor( noninitticks * haste + 0.5 );
				totalticks = cs.binit and (noninitticks+1) or noninitticks;
				shownticks = shownticks + (totalticks - cs.ticks);
				-- shownticks may not exeed 15 (max box elements)
				shownticks = (shownticks > 15) and 15 or shownticks;
			end
		end
...
function Gnosis:SetupBarString( cb, cfg, fCurTime, bDoResize )
	...
	if( cfg.alignment ~= "FREE" and cfg.forcefreealign ~= true and cfg.bResizeLongName and bDoResize ) then
		...
		while ctextmax < cb.ctext:GetStringWidth() do
			...
			if( refactor > 0.95 ) then
				refactor = 0.95;	-- reduce required iterations
			end
