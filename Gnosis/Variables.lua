...
--[[ -- top slightly cut off, therefore using above textures
Gnosis.tRuneTexture = {
...
-- list of spells to compute current haste
Gnosis.tBaseSpellList = {
	DEATHKNIGHT = nil,
	DRUID		= { id = 20484, ct = 2000 },	-- Rebirth, 2.0s base casttime
	HUNTER		= nil,
	MAGE		= { id =   118, ct = 2000 },	-- Polymorph, 2.0s base casttime
	PALADIN		= nil,
	PRIEST		= { id =  2061, ct = 1500 },	-- flash heal, 1.5s base casttime
	ROGUE		= nil,
	SHAMAN		= nil,
	WARLOCK		= { id =  6201, ct = 3000 },	-- Create Healthstone, 3.0s base casttime
	WARRIOR		= nil,
};
...
function Gnosis:StartupVariables()
	...
	self.tBaseSpell = self.tBaseSpellList[self.class];

	self.curincombattype = 3;		-- set as out of combat

	local _, _, _, toc = GetBuildInfo();
	-- which spells still reset swing timer? are there new ones? no more spells which reset the swing timer???
	if( toc >= 40000 ) then
	...
	self.tCastbarDefaults = {
		...
		bShowShield = false,	-- show shielded icon for non-intteruptible casts
