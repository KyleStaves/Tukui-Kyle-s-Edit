...
function Gnosis:CreateStatusBar( parent )
	...
	sbf.SetOrientation = function( this, orientation, bInvDir )
		...
		-- TUKUI SKIN
		local barBg = CreateFrame("Frame", nil, sbf)
		barBg:Point("TOPLEFT", sbf, "TOPLEFT", -2, 2)
		barBg:Point("BOTTOMRIGHT", sbf, "BOTTOMRIGHT", 2, -2)
		barBg:SetTemplate("Default")
		barBg:CreateShadow("Default")
		barBg:SetBackdropBorderColor(unpack(C["media"].altbordercolor))
		barBg:SetFrameStrata(sbf:GetFrameStrata())
		barBg:SetFrameLevel(sbf:GetFrameLevel() - 1)
		-- END TUKUI SKIN
	end
