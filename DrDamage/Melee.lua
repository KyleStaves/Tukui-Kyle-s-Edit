...
function DrDamage:MeleeCalc( name, rank, tooltip, modify, debug )
	...
	calculation.mastery = GetMastery()
	calculation.spec = GetPrimaryTalentTree()
	--[===[@debug@
	calculation.spec = debug or GetPrimaryTalentTree()
	--@end-debug@]===]
	...
	if calculation.group == "Ranged" then
		...
		calculation.critM = (baseSpell.SpellCrit or baseSpell.SpellCritM) and 0.5 or 1
	...
	if settings.Custom then
		...
		calculation.mastery = math_max(0, calculation.mastery + self:GetRating("Mastery", settings.MasteryRating, true))
	end
	...
	calculation.mastery = GetSpellInfo(mastery) and calculation.mastery or 0
	--[===[@debug@
	calculation.mastery = debug and 8 or calculation.mastery
	--@end-debug@]===]
	...
	local baseCrit = (baseSpell.SpellCrit or baseSpell.SpellCritM) and 0.5 or 1
	...
	if settings.TargetLevel > 0 or not UnitIsPlayer("target") then
		if not baseSpell.SpellHit or baseSpell.Avoidable then
			...
			if not calculation.ranged and not baseSpell.Unavoidable then
				...
				elseif deltaLevel >= 0 then
					--Dodge chance: 0 = 5%, +1 = 5.5%, +2 = 6.0%, +3 = 6.5%
					--Parry chance: 0 = 5%, +1 = 5.5%, +2 = 6.0%, +3 = 14%
					calculation.dodge = 5 + deltaLevel * 0.5 - (calculation.dodgeMod or 0)
					calculation.parry = 5 + deltaLevel * 0.5 + math_floor(deltaLevel/3) * 7.5 - (calculation.parryMod or 0)
				end
	...
	if settings.Resilience > 0 then
		...
		calculation.dmgM = calculation.dmgM * math_max(0, 1 - 0.01 * settings.Resilience / self:GetRating("Resilience"))
		if calculation.E_dmgM then
			calculation.E_dmgM = calculation.E_dmgM * math_max(0, 1 - 0.01 * settings.Resilience / self:GetRating("Resilience"))
		end
		if calculation.extraWeaponDamage_dmgM then
			calculation.extraWeaponDamage_dmgM = calculation.extraWeaponDamage_dmgM * math_max(0, 1 - 0.01 * settings.Resilience / self:GetRating("Resilience"))
		end
	end
		...
		if cp > 0 then
			if spell.PerCombo then
				...
				if baseSpell.TicksPerCombo then
					minDam = (baseSpell.DotHits + baseSpell.TicksPerCombo * (cp - 1)) * minDam
					maxDam = (baseSpell.DotHits + baseSpell.TicksPerCombo * (cp - 1)) * maxDam
					eDuration = eDuration + baseSpell.Ticks * (cp - 1)
					baseDuration = baseDuration + baseSpell.Ticks * (cp - 1)
				end
			end
...
function DrDamage:MeleeTooltip( frame, name, rank )
	...
	if CalculationResults.Stats then
		...
		local masA = CalculationResults.NextMastery and CalculationResults.NextMastery > 0 and (CalculationResults.Stats * 0.01 / CalculationResults.NextMastery * self:GetRating("Mastery", nil, true ))
		...
		local rating = CalculationResults.NextHit and CalculationResults.NextHit > 0 and (baseSpell.SpellHit and self:GetRating("Hit", nil, true) or self:GetRating("MeleeHit", nil, true))
		local hitA = rating and (CalculationResults.Stats * 0.01 / CalculationResults.NextHit * rating)
		...
		if settings.Next then
			...
			if masA then frame:AddDoubleLine("+1 " .. L["Mastery"] .. " (" .. self:GetRating("Mastery") .. "):", "+" .. DrD_Round(CalculationResults.NextMastery, 2), rt, gt, bt, r, g, b ) end
			...
			if hitA then frame:AddDoubleLine("+1% " .. L["Hit"] .. " (" .. self:GetRating("MeleeHit") .. "):", "+" .. DrD_Round(CalculationResults.NextHit, 2), rt, gt, bt, r, g, b ) end
		end
