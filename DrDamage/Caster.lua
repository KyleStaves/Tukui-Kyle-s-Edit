...
function DrDamage:CasterCalc( name, rank, tooltip, modify, debug )
	...
	calculation.baseRegen, calculation.manaRegen = GetManaRegen("player")	--Mana regen
	calculation.regenRatio = calculation.manaRegen / calculation.baseRegen
	...
	if settings.Custom then
		if settings.CustomAdd then
			...
			--Mana Regen (Spirit/Intellect)
			local baseRegen = GetUnitManaRegenRateFromSpirit("player")
			calculation.manaRegen = calculation.manaRegen - baseRegen * calculation.regenRatio
	...
	if settings.ManaConsumables and not UnitDebuff("player", potion_sickness) then
		calculation.playerMana = calculation.playerMana + (9250+10750)/2
	end
	...
	if not calculation.critM_custom then
		local baseCrit = baseSpell.MeleeCrit and 1 or calculation.casterCrit and 0.995 or 0.5
	...
	if settings.Resilience > 0 and not healingSpell then
		...
		calculation.dmgM = calculation.dmgM * (1 - math_min(1, 0.01 * settings.Resilience / self:GetRating("Resilience")))
	end
...
DrD_DmgCalc = function( baseSpell, spell, nextCalc, hitCalc, tooltip )
	...
	if sTicks then
		...
		elseif calculation.eDot or calculation.hybridDotDmg then
			...
			if calculation.haste > 1 and not baseSpell.NoDotHaste then
				sTicks = sTicks / calculation.haste
			end
	...
	if nextCalc then
		...
		if tooltip then
			...
			if perHit and ticks then
				...
				if sTicks then
					CalculationResults.Ticks = 		DrD_Round( sTicks, 2 )
				end
			end
...
function DrDamage:CasterTooltip( frame, name, rank )
	...
	if CalculationResults.Stats then
		...
		local rating = CalculationResults.NextHit and (CalculationResults.NextHit > 0.0) and (baseSpell.MeleeHit and self:GetRating("MeleeHit", nil, true) or self:GetRating("Hit", nil, true))
		...
		if settings.Next then
			...
			if hitA then frame:AddDoubleLine("+1% " .. L["Hit"] .. " (" .. DrD_Round(rating,2).. "):", "+" .. DrD_Round(CalculationResults.NextHit, 2), rt, gt, bt, r, g, b ) end
		end
