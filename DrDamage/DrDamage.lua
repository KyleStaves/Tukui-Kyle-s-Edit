...
-- GLOBALS: LibStub, IsAddOnLoaded, Lunar, Macaroon, Nurfed, nUI_DrDamageIntegration, GetTalentTabInfo, CreateFrame, GameFontNormal, UnitRace, InterfaceOptionsFrame, table
			...
			if spells then
				if type(spells) == "table" then
					for i=1,#spells do
						if not tonumber(spells[i]) then
							...
							--[===[@debug@
								...
								self:Print("BuffID lookup failed: " .. spells[i])
							--@end-debug@]===]
							end
							spells[i] = nil
						...
						--[===[@debug@
							...
							self:Print("BuffID removed: " .. spells[i])
						--@end-debug@]===]
						end
					end
				else
					...
					--[===[@debug@
						...
						self:Print("BuffID removed: " .. (spells or temp))
					--@end-debug@]===]
					end
				end
			end
...
function DrDamage:GeneralOptions()
	...
	for t = 1, GetNumTalentTabs() do
		...
		talentTable[("Tab" .. t)] = {
			type = 'header',
			name = --[["|T" .. select(2,GetTalentTabInfo(t)) .. ":30:30:-7:0|t" ..--]] select(2,GetTalentTabInfo(t)),
			order = 2 + (t-1) * 49,
		}
	end
...
local ratingTypes = {
	...
	["Haste"] = 10,
	["MeleeHaste"] = 10,
	["Resilience"] = 9.58333301544189,
	["Expertise"] = 2.34483,
	["Mastery"] = 14,
	["ArmorPenetration"] = 4.268293
}
...
local baseHit = { [-4] = 100, [-3] = 98, [-2] = 96, [-1] = 95.5, [0] = 95, 94.5, 94, 92, 90, 88, 86, 84, 82, 80, 78 }
...
function DrDamage:StatCalc( calculation, baseSpell )
	...
	if calculation.caster and (calculation.int ~= 0 or calculation.spi ~= 0) then
		...
		local regenCoeff = (baseRegen - 0.001) / (spi * int^(1/2))
		if settings.customStats then
			calculation.manaRegen = calculation.manaRegen + (0.001 + calculation.spi * calculation.int^(1/2) * regenCoeff) * regenRatio
			calculation.baseInt = calculation.int
			calculation.baseSpi = calculation.spi
		else
			if not calculation.baseInt then
				calculation.manaRegen = calculation.manaRegen - baseRegen * regenRatio
			else
				calculation.manaRegen = calculation.manaRegen - (0.001 + calculation.baseSpi * calculation.baseInt^(1/2) * regenCoeff) * regenRatio
			end
			calculation.baseInt = math_max(0, (calculation.baseInt or int) + calculation.int)
