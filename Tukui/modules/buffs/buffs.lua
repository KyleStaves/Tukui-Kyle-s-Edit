...
local function UpdateWeapons(button, slot, active, expiration)
	if not button.texture then
		...
		button.time:SetFont(C.media.pixelfont, 10, "OUTLINE")
...
local function UpdateAuras(header, button, weapon)
	if(not button.texture) then
		...
		button.count:SetFont(C.media.pixelfont, 10, "OUTLINE")
...
ScanAuras(CreateAuraHeader("HELPFUL", "TOPRIGHT", -165, -30))
ScanAuras(CreateAuraHeader("HARMFUL", "TOPRIGHT", -165, -160))
