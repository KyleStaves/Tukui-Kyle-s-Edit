...
local ScaleFunctionsDamage = { DummyFunction, ScaleFunctionByElite, ScaleFunctionByTarget, ScaleFunctionByThreatHigh, ScaleFunctionByActiveDebuffs, ScaleFunctionByEnemy,ScaleFunctionByNPC, ScaleFunctionByRaidIcon}
local ScaleFunctionsTank = { DummyFunction, ScaleFunctionByElite, ScaleFunctionByTarget, ScaleFunctionByThreatLow, ScaleFunctionByActiveDebuffs, ScaleFunctionByEnemy, ScaleFunctionByNPC, ScaleFunctionByRaidIcon}
...
local function ColorFunctionsDamage(unit)
	if unit.threatValue > 1 then return LocalVars.ColorAttackingMe
	elseif unit.threatValue == 1 then return LocalVars.ColorAggroTransition
	else return LocalVars.ColorAttackingOthers end
end

local function ColorFunctionsTank(unit)
	if unit.threatValue > 2 then return LocalVars.ColorAttackingMe
...
local function ThreatColorDelegate(unit)
	...
	if LocalVars.ColorDangerGlowOnParty and unit.reaction == "FRIENDLY" then
		if GetThreatCondition(unit.name) then return 1, 0, 0, 1 end
	end
...
local function OnUpdateDelegate(plate, unit)
	...
	if LocalVars.ClassEnemyIcon or LocalVars.ClassPartyIcon then Widgets.ClassIcon:Update(unit, LocalVars.ClassPartyIcon)  end
