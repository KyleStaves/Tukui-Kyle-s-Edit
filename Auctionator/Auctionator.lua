...
function EnableDisableDElogging (verbose, delay)
	...
	if (AUCTIONATOR_SAVEDVARS and AUCTIONATOR_SAVEDVARS.LOG_DE_DATA) then
...
function Atr_DropDown1_OnShow (self)
	UIDropDownMenu_Initialize		(self, Atr_DropDown1_Initialize);
	UIDropDownMenu_SetSelectedValue	(self, MODE_LIST_ACTIVE);
end

-----------------------------------------

function Atr_DropDown1_Initialize(self)
	Atr_Dropdown_AddPick (self, ZT("Active Items"),	MODE_LIST_ACTIVE,	Atr_DropDown1_OnClick);
	Atr_Dropdown_AddPick (self, ZT("All Items"),	MODE_LIST_ALL,		Atr_DropDown1_OnClick);
end
